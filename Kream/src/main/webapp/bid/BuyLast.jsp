...
<%@ page import="java.text.*" %>
  ...
  <script src = "<%=request.getContextPath()%>/js/jquery-3.6.0.min.js"></script>
   ...
   <link rel = "stylesheet" href = "/Kream/css/last.css">
...
%>
   ...
   <%--  // 판매입찰계속 버튼 클릭시 이벤트 (판매입찰 가격) 
	    $(document).on("click", ".btn_full", function(e){
	  	  ...
	  	  if($('.btn_full').attr("check") != "checked"){
		    	...
		    	var amout = $('.input_amount').val();
		    	location.href = '/Kream/buyOrder.do?amout=' + amout + '&sizeId=<%=sizeId%>&prod_id=<%=prodid%>';
		  }//if문 끝
	    });//판매입찰계속 버튼 클릭시 이벤트 끝
          --%>
 ...
 $(document).ready(function(){
	 ...
	 temp_bid = ("<%=bid_price%>").replace(",", "").trim();
	  ...
	  if(<%=bid_price%>!= 0 ){
		 $('.item_link2').trigger('click');  //가격있으면 즉시 구매
	 } else if(<%=bid_price%>== 0  ){
		 $('.item_link2').unbind('click').css('pointer-events','none');
		 $('.item_link').trigger('click');   //구매입찰
	 }  
 ...
 $(function() { 
	...
	//구매입찰가격이 즉시구매보다 클 경우 자동으로 즉시구매로 이동
	   $(document).on('click','.deadline_tab', function() {
	    	var amout = $('.input_amount').val();
	    	//var unit = $('.input_amount').val();
	    	 
	    	if(amout > <%=bid_price%> && <%=bid_price%>!= 0 ){
		    	
	    		$('.item_link2').trigger('click'); //즉시구매
	    	} 
...
out+="           <dt class='price_now_title'> 구매 희망가 </dt>                                               ";                                                                                     
out+="           <dd class='price7'>                                                                       ";                                                                                   
...
//	 out+="	      <dd class='lastprice2'>                                     "     ;
	 <%-- out+="	        <span class='amount lastpr'><%=bid_price %></span>               "     ;   --%>
//	 out+="	        <span class='launit'> 원</span>                         "      ;
//	 out+="	       </dd>                                                 "     ;
	 out+="	    </dl>                                                    "     ;
	 out+="	  </div>                                                     "     ;//<!-- class="price_total" -->
	 out+="<a  href='/Kream/DrbuyOrder.do?prod_id=<%=prodid%>&bid_price=" + temp_bid + "&sizeId=<%=sizeId%>' class='solid'>즉시 구매 계속</a>";
	 out+="	 </div>                                                      "     ;//<!-- class="buy_total_price" -->
...
<div class="instant_group">
  <div class="tab_area">
    <ul class="tab_list">
       <li class="item">
          <a class="item_link" onclick="proc1()"><span class="bidname">구매입찰</span></a>
        </li>
        <li class="item on">
          <a class="item_link2" onclick="proc2()"><span class="bidname">즉시 구매</span></a>
        </li>
    </ul>
